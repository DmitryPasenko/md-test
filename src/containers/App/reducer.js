...
import {APPLY, CANCEL, DELETE, TOGGLE_OPTION, DELETE_ALL } from '../App/constants';
...
function appReducer(state = initialState, action) {
    ...
    switch (action.type) {
        case APPLY:
            return state.update(action.section, (list) => list.map((item)=>item.set('selected',item.get('checked'))));
        case CANCEL:
            return state.update(action.section,  (list) => list.map((item)=>item.set('checked', false)));
        case DELETE_ALL:
        {
            return initialState
        }
        case DELETE:
            {
                const idx = state.get(action.section).findIndex((item) =>item.get('id') === action.id);
                return  state.update(action.section, (list) =>  list.setIn([idx,'selected'], false).setIn([idx,'checked'], false));
            }
        case TOGGLE_OPTION:
            return state.update(action.section,  (list) =>  list.setIn([action.index,'checked'], !list.getIn([action.index,'checked'])));
        default:
            return state;
    }
}
